...
export const ARPreviewOverlay: React.FC<ARPreviewOverlayProps> = ({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // 按二维码位姿放置锚点：模型底边中点贴合二维码中心，模型立于二维码平面
      const applyPose = (pose: QRPose) => {
        ...
        if (f.lengthSq() < 1e-8) {
          f = new THREE.Vector3(0, 1, 0).applyQuaternion(pose.quaternion);
        }
        f.normalize();
        ...
        try {
          ...
          if (ctx) {
            ...
            if (code) {
              ...
              if (matches) {
                const pose = estimateQRPose(corners, vw, vh, qrSizeCmRef.current, CAMERA_FOV);
                if (pose) return { kind: 'match', pose };
              }
              return { kind: 'other' };
            }
